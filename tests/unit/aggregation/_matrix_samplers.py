...
class NormalSampler(MatrixSampler):
    ...
    def __call__(self) -> Tensor:
        U = _sample_orthonormal_matrix(self.m, dtype=self.dtype)
        Vt = _sample_orthonormal_matrix(self.n, dtype=self.dtype)
        S = torch.diag(torch.abs(torch.randn([self.rank], dtype=self.dtype)))
...
class StrongSampler(MatrixSampler):
    ...
    def __call__(self) -> Tensor:
        v = torch.abs(torch.randn([self.m], dtype=self.dtype))
...
class StrictlyWeakSampler(MatrixSampler):
    ...
    def __call__(self) -> Tensor:
        u = torch.abs(torch.randn([self.m], dtype=self.dtype))
        split_index = torch.randint(1, self.m, []).item()
        shuffled_range = torch.randperm(self.m)
