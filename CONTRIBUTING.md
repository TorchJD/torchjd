...
3) Install the project and its dependencies. From the root of torchjd, run:
   ...
   > [!NOTE] If it does not create this `.venv` folder, you can try to run `pdm venv create`,
   followed by `pdm use .venv/bin/python`, and install the project by re-running `pdm install
   --frozen-lockfile`.

   > [!NOTE] The python version that you should specify in your IDE is
